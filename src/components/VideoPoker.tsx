...
import { BetRail, OutcomeBanner, Paytable, PlayingCard, StakeReadout } from "./ui";
...
export function VideoPoker() {
  ...
  const [message, setMessage] = useState("Deal five. Hold the keepers.");
  const [evalLabel, setEvalLabel] = useState("");

  function deal() {
    if (phase === "hold") return;
    if (!spend("poker", `Video poker ${formatChips(chip)}`, chip)) {
      setMessage("Not enough chips to deal.");
      return;
    }
    ...
    setMessage(preview.rank === "nothing" ? "Choose cards to hold, then draw." : `${preview.label} — hold or draw.`);
  }
  ...
  function draw() {
    ...
    setMessage(returned > 0 ? `${result.label} — paid ${formatChips(returned)}` : "Nothing. The deck resets.");
  }
        ...
        <div className="poker-hand">
          {hand.map((card, i) => (
            <button
              ...
              disabled={phase !== "hold"}
            >
              <PlayingCard card={card} delay={i * 60} />
